...
class Speech_Emotion_Recognition:
    def __init__(
        ...
        self.din = device_input
        self.dout= device_output
        self.hout= host_output
        self.outs= output_shape
    ...
    def mel_inference(self):
        ...
        print("inference threadis is on-running")
        ...
        while not self.stop_event.is_set():
           ...
           # print("5", norm_data_3.shape)
            ...
            hin = np.array(data_norm.cpu().numpy(), dtype=np.float32, order='C')
            #print("shape, isnan, isinf:", hin.shape, np.isnan(hin).any(), np.isinf(hin).any())
            ctx.push()
            cuda.memcpy_htod_async(self.din, hin, self.engine_stream)
            print(f"preprocessing : {(time.time()-pretime)*1000:.3f}ms")
            prev_time= time.time()
            self.context.execute_async_v2([int(self.din), int(self.dout)], self.engine_stream.handle)
            cuda.memcpy_dtoh_async(self.hout, self.dout, self.engine_stream)
            self.engine_stream.synchronize()
            ctx.pop()
            output = torch.Tensor(self.hout).reshape(self.engine.max_batch_size, self.outs[1])
...
if __name__ == '__main__':
    ...
    engine_name = "SER_fp16.engine" if args.fp16 else "SER_fp32.engine"
    print("now engine: ", engine_name)

    TRT_LOGGER = tensorrt.Logger(tensorrt.Logger.VERBOSE)
    with open(engine_name, "rb") as f:
        engine = tensorrt.Runtime(TRT_LOGGER).deserialize_cuda_engine(f.read())
    context = engine.create_execution_context()
    
    input_idx = engine.get_binding_index(engine[0])
    output_idx = engine.get_binding_index(engine[1])
    
    context.set_binding_shape(input_idx, (1, 3, 128, 128))
    
    in_shape = context.get_binding_shape(input_idx) # (1,3,128,128) 
    out_shape = context.get_binding_shape(output_idx) # (1,7) 
    
    in_size = tensorrt.volume(in_shape) * np.dtype(np.float32).itemsize 
    out_size = tensorrt.volume(out_shape) * np.dtype(np.float32).itemsize 
    
    d_input = cuda.mem_alloc(in_size) 
    d_output = cuda.mem_alloc(out_size) 
    h_output = cuda.pagelocked_empty(tensorrt.volume(out_shape), dtype=np.float32)
    #for binding in engine:
    #    if engine.binding_is_input(binding):
    #        input_shape = engine.get_binding_shape(binding)
    #        input_size = tensorrt.volume(input_shape) * np.dtype(np.float32).itemsize * engine.max_batch_size
    #        device_input = cuda.mem_alloc(input_size)
    #    else:
    #        output_shape = engine.get_binding_shape(binding)
    #        host_output = cuda.pagelocked_empty(tensorrt.volume(output_shape) * engine.max_batch_size, dtype=np.float32)
    ...
    ctx.push()
    cuda.memcpy_htod_async(d_input, dummy, engine_stream)
    context.execute_async_v2([int(d_input), int(d_output)], engine_stream.handle)
    cuda.memcpy_dtoh_async(h_output, d_output, engine_stream)
    engine_stream.synchronize()
    ctx.pop()
    print("Warming up End!")

    ser = Speech_Emotion_Recognition(engine, engine_stream, d_input, d_output, h_output, out_shape, context)
